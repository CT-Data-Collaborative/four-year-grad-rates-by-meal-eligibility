...
##################################################################
#
# Processing Script for Four-Year-Grad-Rates-by-Meal-Eligibility
...
four_yr_gr <- merge(four_yr_gr_grad, four_yr_gr_nongrad, by = c("District", "Free/Reduced Price Meal Eligibility", "Total Cohort Count", "Year"))
...
backfill_years <- expand.grid(
  ...
  `Free/Reduced Price Meal Eligibility` = unique(four_yr_gr$`Free/Reduced Price Meal Eligibility`),
  `Year` = years
)
...
complete_four_yr_gr <- complete_four_yr_gr[!with(complete_four_yr_gr, is.na(complete_four_yr_gr$`Free/Reduced Price Meal Eligibility`)),]
...
test <- complete_four_yr_gr_long[,c("District", "Year", "Free/Reduced Price Meal Eligibility", "Variable")]
test2<-test[duplicated(test), ]

#Write CSV
write.table(
  complete_four_yr_gr_long,
  file.path(path_to_top_level, "data", "four_year_grad_rate_by_meal_eligibility_2011-2019.csv"),
  sep = ",",
  row.names = F
)
